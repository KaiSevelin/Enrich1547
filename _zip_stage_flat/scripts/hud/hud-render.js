...
function buildCategoryContent(data, activeCategory, deps = {}) {
    ...
    switch (activeCategory) {
        ...
        case "maneuvers":
            return (() => {
                const preRows = buildTreeList(data.maneuvers, (maneuver) => {
                    const disabledAttr = maneuver.disabled ? " disabled" : "";
                    const activeClass = maneuver.selected ? " is-active" : "";
                    const title = escapeHtml(maneuver.tooltip || "");
                    const subtitle = maneuver.reason || maneuver.summaryLine || `${maneuver.costSummary} | ${maneuver.effectSummary}`;
                    const detail = maneuver.detailLine || "";
                    return `
                        <li class="hud-tree-item" title="${title}">
                            <button
                                type="button"
                                class="hud-action-row${activeClass}"
                                data-hud-pre-maneuver="${escapeHtml(maneuver.id)}"
                                ${disabledAttr}
                            >
                                ...
                                <span class="hud-tree-value">${escapeHtml(maneuver.selected ? "Selected" : maneuver.costSummary)}</span>
                            </button>
                        </li>
                    `;
                });

                const fullTurnRows = buildTreeList(data.fullTurnManeuvers, (maneuver) => {
                    const disabledAttr = maneuver.disabled ? " disabled" : "";
                    const activeClass = maneuver.selected ? " is-active" : "";
                    const title = escapeHtml(maneuver.tooltip || "");
                    const subtitle = maneuver.reason || maneuver.summaryLine || `${maneuver.costSummary} | ${maneuver.effectSummary}`;
                    ...
                    return `
                        <li class="hud-tree-item" title="${title}">
                            <button
                                type="button"
                                class="hud-action-row${activeClass}"
                                data-hud-full-turn-maneuver="${escapeHtml(maneuver.id)}"
                                ${disabledAttr}
                            >
                                <span class="hud-row-main">${escapeHtml(maneuver.name)}</span>
                                <span class="hud-row-sub">${escapeHtml(subtitle)}</span>
                ...
                if (!data.maneuvers.length && !data.fullTurnManeuvers.length) {
                    return `<div class="hud-empty-row">No legal pre or full-turn maneuvers</div>`;
                }
                ...
                return `
                    ...
                    ` : ""}
                    ${data.fullTurnManeuvers.length ? `
                        <div class="hud-tree-block">
                            <div class="hud-section-title">Full-Turn Maneuvers</div>
                            <ul class="hud-list hud-tree-list hud-list-scroll hud-single-scroll">
                                ${fullTurnRows}
                            </ul>
                        </div>
