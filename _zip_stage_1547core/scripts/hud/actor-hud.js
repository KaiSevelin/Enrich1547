...
function getAmmoRangeData(item) {
    ...
    if (!loadedAmmoId) {
        return { rangeModifier: null, rangeOverride: null };
    }
    ...
    return {
        rangeModifier: ammoSource.rangeModifier ?? null,
        rangeOverride: ammoSource.rangeOverride ?? null
    };
}
...
function applyAmmoRangeBands(rangeBands, ammoRangeData) {
    if (ammoRangeData?.rangeOverride && typeof ammoRangeData.rangeOverride === "object") {
        return normalizeRangeBandOrder({
            shortRange: Number(ammoRangeData.rangeOverride.shortRange),
            longRange: Number(ammoRangeData.rangeOverride.longRange),
            maxRange: Number(ammoRangeData.rangeOverride.maxRange)
        });
    }
    if (ammoRangeData?.rangeModifier && typeof ammoRangeData.rangeModifier === "object") {
        return normalizeRangeBandOrder({
            shortRange: (Number.isFinite(rangeBands.shortRange) ? rangeBands.shortRange : 0) + (Number(ammoRangeData.rangeModifier.shortRange) || 0),
            longRange: (Number.isFinite(rangeBands.longRange) ? rangeBands.longRange : (Number.isFinite(rangeBands.shortRange) ? rangeBands.shortRange : 0)) + (Number(ammoRangeData.rangeModifier.longRange) || 0),
            maxRange: (Number.isFinite(rangeBands.maxRange) ? rangeBands.maxRange : (Number.isFinite(rangeBands.longRange) ? rangeBands.longRange : 0)) + (Number(ammoRangeData.rangeModifier.maxRange) || 0)
        });
    }
...
function getAmmoSummary(item) {
    ...
    const addDiceSummary = getStringProp(itemProps, ["AddDiceSummary", "AddDice"])
        || getStringProp(sourceData, ["addDiceSummary", "addDice"]);
    const tagsSummary = getStringProp(itemProps, ["TagsSummary", "Tags"])
        || getStringProp(sourceData, ["tagsSummary", "tags"]);
    const modifiersSummary = getStringProp(itemProps, ["ResultModifiersSummary", "ResultModifiers"])
        || getStringProp(sourceData, ["resultModifiersSummary", "resultModifiers"]);
...
async function executeWeaponReloadAction(weaponId, actor, summary) {
    ...
    if (weapon.loadedAmmoId && Number.isFinite(weapon.loadedAmmoQuantity) && weapon.loadedAmmoQuantity > 0 && Number(weapon.ammoLoaded ?? 0) > 0) {
        ui.notifications?.info?.("Weapon is already loaded.");
        return;
    }
            ...
            return `
                <li class="hud-tree-item hud-weapon-card">
                    ...
                    <div class="hud-weapon-action-strip">
                        ...
                        ` : ""}
                        ${!weapon.ready && !weapon.equipped ? `
                            <button
                                type="button"
                                class="hud-mini-button"
                                data-hud-weapon-ready="${escapeHtml(weapon.id)}"
                            >
                                Ready
                            </button>
                        ` : ""}
