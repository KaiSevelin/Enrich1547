...
function applyAmmoRangeEffects(rangeBands, ammo) {
    const override = ammo?.rangeOverride ?? null;
    if (override && typeof override === "object") {
        return normalizeRangeBands({
            shortRange: Number(override.shortRange),
            longRange: Number(override.longRange),
            maxRange: Number(override.maxRange),
        });
    }

    const modifier = ammo?.rangeModifier ?? null;
    if (modifier && typeof modifier === "object") {
        return normalizeRangeBands({
            shortRange: (rangeBands.shortRange ?? 0) + (Number(modifier.shortRange) || 0),
            longRange: (rangeBands.longRange ?? rangeBands.shortRange ?? 0) + (Number(modifier.longRange) || 0),
            maxRange: (rangeBands.maxRange ?? rangeBands.longRange ?? 0) + (Number(modifier.maxRange) || 0),
        });
    }
...
function normalizeAmmoItem(ammo) {
    ...
    return {
        ...
        rangeModifier: source.rangeModifier ?? null,
        rangeOverride: source.rangeOverride ?? null,
    };
}
...
function isRangedLikeAttackType(attackType) {
...
function passesProfileGate(maneuver, context) {
    ...
    if (appliesTo === "ranged-attack") {
        return isRangedLikeAttackType(profile.attackType);
    }
...
function passesRangeGate(maneuver, context) {
    const profile = context.profile ?? null;
    if (!profile || !isRangedLikeAttackType(profile.attackType)) return true;
