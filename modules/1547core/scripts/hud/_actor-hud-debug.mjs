...
function evaluateWeaponAttackAction(descriptor, context) {
    ...
    return {
        ...
        rollPreview: {
            ...
            advantageDice: context.summary.rollContext.advantageDice,
            riskDice: context.summary.rollContext.riskDice,
            finalFormula: weapon.activeAttackFormula || "-",
            costs: {},
...
async function executeWeaponAttackAction(descriptor, context, evaluation) {
    ...
    const profileFormula = evaluation?.resolvedSource?.weapon?.activeAttackFormula ?? "";
    if (!context.primaryTarget) {
        if (!profileFormula) {
            ui.notifications?.warn?.("This weapon does not have an attack formula to roll.");
            return;
        }
        const roll = await new Roll(profileFormula).evaluate({ async: true });
        const speaker = ChatMessage.getSpeaker({ actor: context.actor, token: context.token?.document });
    ...
    const distanceSquares = evaluation?.resolvedSource?.attackState?.distanceSquares ?? getChebyshevDistanceSquares(context.token, context.primaryTarget);
    try {
        const result = await declareAttack({
            ...
            weapon: weaponItem,
            profileId: evaluation?.resolvedSource?.weapon?.activeAttackProfileId ?? null,
            distanceSquares
        });
        if (result?.cancelled) {
...
async function executeWeaponReloadAction(weaponId, actor, summary) {
    if (!isTruthyLike(summary?.fullTurnAvailable)) {
