...
- [ ] **Advancement two-step (Regression):** clicking a choice row **arms** it and shows a small **Add** button on the **right**; clicking **Add** (or the armed row again) confirms; clicking a different row moves the arm. Option text stays fully visible.
...
- [ ] **Player cancel (Regression):** a **non-GM player** clicking **Cancel & Delete** removes the character — the **GM client deletes it** over the socket (no `lacks permission to delete Actor` error); requires a GM online.
...
- [ ] **Critical maneuver conditions (Regression):** committing **Lock / Throw / Constrict / Choke** applies the condition (**Locked / Prone / Grappled / Choking Hold**) to the **target** as an ActiveEffect — routed via the GM if the attacker can't write the target. The target then rolls combat with **+1 disadvantage** die (Choking Hold also cancels advantage). _(Positional/ongoing-damage parts of those maneuvers are named in the chat card for the GM to apply.)_
- [ ] **Escape maneuvers (Regression):** the escape maneuvers (**Break Grapple** 1 DEX, **Slip The Lock** 1 STR, **Break The Choke** 1 STR, **Stand Up** free) are **learned items** on the sheet (granted at chargen; run **Setup Data** so they're in the compendium). They appear in the HUD maneuver list **only while you hold the matching condition**, sorted **first** with a **red border**; **disabled/greyed** when you can't afford the point. Clicking one **spends the stat point and removes the condition** (Stand Up is free; "forgo all movement" is a manual note). An actor can only perform maneuvers/skills that are items on its sheet.
- [ ] **Player attacks GM/NPC — no console errors (Regression):** no `lacks permission to update Item ... ActorDelta` (SkillTree createItem only writes for the item's owner now) and no `DiceTerm.fromParseNode ... reading 'name'` (the custom 1547 dice register independently of Dice So Nice, so roll formulas always parse). Rolls render with the 1547 dice faces.
