...
- [ ] **Condition escape (Regression):** select a token with **Grappled/Locked/Choking Hold** → an **Escape** button shows in the HUD action row. Clicking it spends the token's **reaction** and rolls the held stat (best of the allowed: Str / Str-or-Dex; Choke at disadvantage) **vs the inflictor's Strength** — chat shows both totals; a higher held total **removes the condition** (grappler wins ties). With the reaction already spent, escape warns "no reaction left". **Prone** shows **Stand up** (no roll) → confirm → removes Prone (movement forgone, not automated).
